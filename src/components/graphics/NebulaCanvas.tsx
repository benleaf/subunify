...
const NebulaCanvas = ({ width = 800, pointMultiplier = 3 }: Props) => {
    ...
    useEffect(() => {
        ...
        const render = () => {
            ctx.fillStyle = `rgba(0, 0, 0, ${baseSpeed * 40})`;
            ...
            const gradient = ctx.createRadialGradient(centerX, centerY, 0.1, centerX, centerY, width / 10);

            gradient.addColorStop(0, "#ffff");
            gradient.addColorStop(0.02, "#ffffff22");
            gradient.addColorStop(0.1, "#ffffff0f");
            gradient.addColorStop(0.3, "#ffffff09");
            gradient.addColorStop(1, "#0000");

            ctx.fillStyle = gradient
            ctx.beginPath();
            ctx.arc(centerX, centerY, width / 10, 0, Math.PI * 2);
            ctx.fill();
